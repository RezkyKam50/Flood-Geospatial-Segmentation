...
from utils.dataload import get_loader
...
def parse_arguments():
    ...
    parser.add_argument('--model', type=str, default='unet', help='Model to use for training (unet, prithvi_unet, prithvi, ensemble)')
...
def test(model, loader, epoch, criterion, set_name='valid'):
    model.eval()
    metricss = {}
    index = 0
    ...
    logger.info(f"{set_name}: Loss: {loss / index:.4f}")
    ...
    writer.add_scalar(f"Loss/{set_name}", loss / index, epoch)
...
def main(args):
    ...
    match args.model:
        ...
        case 'prithvi_unet':
            ...
            logger.info(f"Using Prithvi-UNet...")
        case 'prithvi':
    ...
    def train_model_for_n_epochs(model, train_loader, optimizer, criterion, scheduler, epoch_range):
        for epoch in range(*epoch_range):
            ...
            if (epoch+1) % args.test_interval == 0:
                test(model, valid_loader, epoch, criterion)
    ...
    # Test the model
    test(model, test_loader, args.epochs, criterion, 'test')
    test(model, bolivia_loader, args.epochs, criterion, 'bolivia')
