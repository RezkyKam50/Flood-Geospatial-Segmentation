...
USED_BANDS = (1,2,3,8,11,12) # Sentinel-2 Only bands
...
def processAndAugment(data):
    ...
    norm = transforms.Normalize(S2_MEANS, S2_STDS)
    img = norm(img)
...
IMAGE_DIR_S2 = 'data/S2L1CHand' # 6 sentinel-2 bands
DATASET_DIR = 'splits'

def load_flood_data_s2(path, dataset_type):
    fpath = os.path.join(path, DATASET_DIR, f'flood_{dataset_type}_data.txt')
    with open(fpath) as f:
        get_img_path = lambda identifier: os.path.join(path, IMAGE_DIR_S2, f"{identifier}_S2Hand.tif")
...
def get_train_loader(data_path, args):
    train_data = load_flood_data_s2(data_path, 'train')
...
def get_test_loader(data_path, type):
    valid_data = load_flood_data_s2(data_path, type)
