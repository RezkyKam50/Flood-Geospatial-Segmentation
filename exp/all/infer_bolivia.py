...
def accumulate_confusion_matrix(prediction, ground_truth, ignore_index=255):
    """
    Accumulate confusion matrix metrics (TP, FP, TN, FN) for binary segmentation.
    This matches the logic from the test() function.
    
    Args:
        prediction: numpy array of predicted labels (H, W)
        ground_truth: numpy array of ground truth labels (H, W)
        ignore_index: label value to ignore (e.g., -1 or 255 for no data)
...
def calculate_metrics_from_confusion(metrics_dict):
    """
    Calculate IoU and accuracy metrics from accumulated confusion matrix.
    This matches the logic from the test() function.
    
    Args:
        metrics_dict: dict with accumulated TP, FP, TN, FN
    
    Returns:
        dict with IOU_floods, IOU_non_floods, Avg_IOU, ACC_floods, ACC_non_floods, Avg_ACC
    """
    TP = metrics_dict['TP']
    FP = metrics_dict['FP']
    TN = metrics_dict['TN']
...
def main():
    ...
    # Initialize confusion matrix accumulation for each model
    ...
    for filename, mask_filename in data_files:
        ...
        for model_type, model in models.items():
            ...
            # Accumulate confusion matrix for this image
    ...
    # Calculate and display overall mIoU statistics using accumulated confusion matrices
    logger.info("=" * 60)
